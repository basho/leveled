...
%% pairs.  This is only used in tests, and when rolling a L0 file on shutdown.
%% New SST files are generally created by sst_newmerge function, when files are
%% being generated by the LSM merge process. 
...
sst_new(RootPath, Filename, Level, KVList, MaxSQN, OptsSST, IndexModDate) ->
    ...
    MaxSlots0 = maxslots_level(Level, OptsSST#sst_options.max_sstslots, false),
    OptsSST0 =
...
sst_newmerge(RootPath, Filename, 
    ...
    PressMethod0 =
        compress_level(Level, OptsSST#sst_options.press_method),
    MaxSlots0 =
        maxslots_level(Level, OptsSST#sst_options.max_sstslots, IsBasement),
    OptsSST0 =
        OptsSST#sst_options{press_method = PressMethod0,
                            max_sstslots = MaxSlots0},
...
sst_newlevelzero(RootPath, Filename, 
    ...
    MaxSlots0 = maxslots_level(0, OptsSST#sst_options.max_sstslots, false),
    OptsSST0 =
        OptsSST#sst_options{press_method = PressMethod0,
                            max_sstslots = MaxSlots0},
    {ok, Pid} = gen_fsm:start_link(?MODULE, [], []),
...
-spec maxslots_level(non_neg_integer(), pos_integer(), boolean())
                        -> pos_integer().
maxslots_level(Level, MaxSlotCount, true) when Level >= ?DOUBLESIZE_LEVEL ->
    2 * MaxSlotCount;
maxslots_level(_Level, MaxSlotCount, _IsBasement) ->
    MaxSlotCount.
...
size_tester(KVL1, KVL2, N) ->
    ...
    OptsSST = 
        #sst_options{press_method=native,
                        log_options=leveled_log:get_opts()},
    {ok, SST1, _KD, _BB} = sst_newmerge(RP, Filename, 
                                KVL1, KVL2, true, ?DOUBLESIZE_LEVEL, 
                                N, OptsSST, false, false),
    ok = sst_close(SST1),
    {ok, SST2, _SKEK, Bloom} =
